...
public class GroupMembershipDao {
    ...
    /**
     * method called by the cache manager when it doesn't have a hit (match) in cache
     * @param theCacheKey the GroupMembershipCacheKey passed to the cache manager
     * @return the value returned from the original DAO method
      */
    public boolean isUserInGroup(GroupMembershipCacheKey theCacheKey) {
        // This will call the original DAO method with the values from the cache-key
        return this.isUserInGroup(theCacheKey.getUserId(), theCacheKey.getGroupId());  // this. is optional
    }
